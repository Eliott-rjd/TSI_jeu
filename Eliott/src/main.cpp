...
static void init()
{
  ...
  third_person_perso(obj[0], &cam); //chargement de la caméra 3eme personne
...
static void keyboard_callback(unsigned char key, int, int)
{
  switch (key)
  {
    ...
    case 32: //Barre d'espace
        //TODO la chute apres le saut (comment faire de la gravité)
...
{   
    if (last_x > (float)x) { //Un mouvement de souris sur le coté, entraine une rotation selon l'axe des y de l'objet
...
void  third_person_perso(objet3d obj, camera* cam)
{
    ...
    //On l'inverse donc (cf shéma à refaire dans le compte rendu).
